...
<div class="space-y-10 animate-in fade-in duration-500">
    ...
    <div class="flex flex-col md:flex-row md:items-end justify-between gap-6 border-b border-vercel-gray-200 dark:border-vercel-gray-800 pb-8">
        <div>
            <h1 class="text-3xl font-black text-vercel-black dark:text-vercel-white tracking-tight"><?php echo __('announcements'); ?></h1>
        ...
        <button onclick="showCreateModal()" class="px-6 py-2.5 bg-vercel-black dark:bg-vercel-white text-white dark:text-vercel-black rounded-md font-bold text-sm hover:bg-vercel-gray-800 dark:hover:bg-vercel-gray-100 transition-all flex items-center gap-2">
    ...
    <div class="space-y-6">
        <?php if (empty($announcements)): ?>
            <div class="py-20 text-center text-vercel-gray-400 font-medium border border-dashed border-vercel-gray-200 dark:border-vercel-gray-800 rounded-lg">No announcements found.</div>
        <?php else: ?>
            <?php foreach ($announcements as $ann): ?>
                <div class="bg-white dark:bg-vercel-gray-900 border border-vercel-gray-200 dark:border-vercel-gray-800 rounded-lg p-8 group hover:border-vercel-black dark:hover:border-vercel-white transition-colors relative">
                    <div class="flex flex-col md:flex-row md:items-start justify-between gap-6 mb-8">
                        <div class="flex-1 min-w-0">
                            <div class="flex items-center gap-3 mb-3">
                                <h3 class="text-xl font-black text-vercel-black dark:text-vercel-white tracking-tight group-hover:underline decoration-vercel-gray-200 dark:decoration-vercel-gray-800 underline-offset-4"><?php echo htmlspecialchars($ann['title']); ?></h3>
                                <span class="px-2 py-0.5 border rounded text-[8px] font-black uppercase tracking-widest
                                    <?php echo $ann['is_active'] ? 'bg-vercel-green-50 dark:bg-vercel-green-900/20 text-vercel-green-600 border-vercel-green-200/50 dark:border-vercel-green-900/40' : 'bg-vercel-gray-50 dark:bg-vercel-gray-800 text-vercel-gray-500 border-vercel-gray-200/50 dark:border-vercel-gray-700'; ?>">
                                    <?php echo $ann['is_active'] ? 'Active' : 'Hidden'; ?>
                                </span>
                            </div>
                            <div class="flex flex-wrap items-center gap-x-6 gap-y-1">
                                <div class="flex items-center gap-1.5 text-[11px] text-vercel-gray-500 dark:text-vercel-gray-400 font-bold italic">
                                    <div class="w-4 h-4 rounded bg-vercel-gray-100 dark:bg-vercel-gray-800 flex items-center justify-center text-[7px] font-black text-vercel-gray-400">
                        ...
                        <div class="flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                            <button onclick="showEditModal(<?php echo htmlspecialchars(json_encode($ann)); ?>)" class="p-2 hover:bg-vercel-gray-100 dark:hover:bg-vercel-gray-800 rounded-md text-vercel-gray-400 hover:text-vercel-black dark:hover:text-vercel-white transition-colors" title="Edit">
                                <i data-lucide="edit-3" class="w-4 h-4"></i>
                            </button>
                            <button onclick="toggleAnnouncement(<?php echo $ann['id']; ?>, <?php echo $ann['is_active'] ? '0' : '1'; ?>)" class="p-2 hover:bg-vercel-gray-100 dark:hover:bg-vercel-gray-800 rounded-md text-vercel-gray-400 hover:text-vercel-black dark:hover:text-vercel-white transition-colors" title="<?php echo $ann['is_active'] ? 'Hide' : 'Show'; ?>">
                                <i data-lucide="<?php echo $ann['is_active'] ? 'eye-off' : 'eye'; ?>" class="w-4 h-4"></i>
                            </button>
                            <button onclick="confirmDelete(<?php echo $ann['id']; ?>)" class="p-2 hover:bg-vercel-gray-100 dark:hover:bg-vercel-gray-800 rounded-md text-vercel-gray-400 hover:text-vercel-red transition-colors" title="Delete">
                                <i data-lucide="trash-2" class="w-4 h-4"></i>
                            </button>
                        </div>
                    </div>

                    <div class="text-sm font-medium text-vercel-gray-600 dark:text-vercel-gray-400 leading-relaxed bg-vercel-gray-100/30 dark:bg-vercel-gray-800/30 p-6 rounded-md border border-vercel-gray-100 dark:border-vercel-gray-800 whitespace-pre-wrap">
    ...
    <?php if ($totalPages > 1): ?>
        <div class="flex items-center justify-between border-t border-vercel-gray-200 dark:border-vercel-gray-800 pt-8 mt-4">
            <div class="text-xs text-vercel-gray-400 font-medium">
                Showing <span class="text-vercel-black dark:text-vercel-white font-bold"><?php echo $offset + 1; ?></span> to <span class="text-vercel-black dark:text-vercel-white font-bold"><?php echo min($total, $offset + $perPage); ?></span> of <span class="text-vercel-black dark:text-vercel-white font-bold"><?php echo $total; ?></span> announcements
            </div>
...
<script>
        ...
        Modal.create({
            ...
            content: `
                <form id="create-ann-form" class="space-y-6 pt-4">
                    ...
                    <div class="flex items-center gap-3 p-4 bg-vercel-gray-50 dark:bg-vercel-gray-800 rounded-md border border-vercel-gray-100 dark:border-vercel-gray-700">
                        ...
                        <span class="text-[11px] font-black text-vercel-black dark:text-vercel-white uppercase tracking-tight">Deploy immediately</span>
                    </div>
                </form>
            `,
            footer: `
            ...
            content: `
                <form id="edit-ann-form" class="space-y-6 pt-4">
                    ...
                    <div class="flex items-center gap-3 p-4 bg-vercel-gray-50 dark:bg-vercel-gray-800 rounded-md border border-vercel-gray-100 dark:border-vercel-gray-700">
                        ...
                        <span class="text-[11px] font-black text-vercel-black dark:text-vercel-white uppercase tracking-tight">Active Visibility</span>
                    </div>
                </form>
            `,
            footer: `
    ...
    async function toggleAnnouncement(id, s) {
        try {
            const res = await API.post('<?php echo SITE_URL; ?>/api/admin/update-announcement.php', {
                id,
                is_active: s
            });
            if (res.success) {
        ...
        Modal.create({
            ...
            onOpen: (m) => {
                m.querySelector('#exec-del-ann').onclick = async () => {
                    ...
                    try {
                        const res = await API.delete('<?php echo SITE_URL; ?>/api/admin/delete-announcement.php', {
                            id
                        });
                        if (res.success) {
                            Toast.success('Wiped');
