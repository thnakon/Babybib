...
<div class="space-y-10 animate-in fade-in duration-500">
    ...
    <div class="flex flex-col md:flex-row md:items-end justify-between gap-6 border-b border-vercel-gray-200 dark:border-vercel-gray-800 pb-8">
        <div>
            <h1 class="text-3xl font-black text-vercel-black dark:text-vercel-white tracking-tight"><?php echo __('bibliography_management'); ?></h1>
    ...
    <div class="flex flex-wrap items-center gap-4">
        ...
        <div class="flex flex-wrap items-center gap-2">
            <select id="filter-type" class="px-3 py-2 bg-white dark:bg-vercel-gray-900 border border-vercel-gray-200 dark:border-vercel-gray-800 rounded-md text-xs font-semibold text-vercel-gray-500 hover:border-vercel-black dark:hover:border-vercel-white transition-all outline-none text-vercel-black dark:text-vercel-white">
            ...
            <?php foreach ($bibliographies as $bib): ?>
                <div class="group hover:bg-vercel-gray-100/50 dark:hover:bg-vercel-gray-800/50 transition-colors p-6">
                    <div class="flex flex-col md:flex-row gap-6">
                        <!-- Icon area -->
                        <div class="w-10 h-10 rounded border border-vercel-gray-200 dark:border-vercel-gray-800 flex items-center justify-center text-vercel-gray-400 group-hover:bg-vercel-black dark:group-hover:bg-vercel-white group-hover:text-white dark:group-hover:text-vercel-black group-hover:border-vercel-black dark:group-hover:border-vercel-white transition-all">
                            <i data-lucide="<?php echo getLucideIcon($bib['rt_icon']); ?>" class="w-4 h-4"></i>
                        </div>

                        <!-- Content area -->
                        <div class="flex-1 min-w-0">
                            <div class="text-sm font-medium text-vercel-black dark:text-vercel-white leading-relaxed mb-3">
                                <?php echo htmlspecialchars(strip_tags($bib['bibliography_text'])); ?>
                            </div>

                            <div class="flex flex-wrap items-center gap-4">
                                <span class="px-2 py-0.5 border border-vercel-gray-100 dark:border-vercel-gray-800 bg-vercel-gray-100 dark:bg-vercel-gray-800 text-vercel-gray-500 rounded text-[10px] font-black uppercase tracking-widest">
                                ...
                                <div class="flex items-center gap-2">
                                    <div class="w-5 h-5 rounded-full bg-vercel-gray-100 dark:bg-vercel-gray-800 flex items-center justify-center text-[8px] font-black text-vercel-gray-400 overflow-hidden">
                                        ...
                                        <?php else: ?>
                                            <?php echo strtoupper(substr($bib['user_name'] ?? 'A', 0, 1)); ?>
                                        <?php endif; ?>
                                    </div>
                                    <span class="text-[11px] text-vercel-gray-500 dark:text-vercel-gray-400 font-bold">@<?php echo htmlspecialchars($bib['username']); ?></span>
                                </div>
                            </div>
                        </div>
                        ...
                        <div class="flex items-start justify-end gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                            <button onclick="viewBibDetails(<?php echo htmlspecialchars(json_encode($bib)); ?>)" class="p-2 hover:bg-vercel-gray-100 dark:hover:bg-vercel-gray-800 rounded-md text-vercel-gray-400 hover:text-vercel-black dark:hover:text-vercel-white transition-colors">
                                <i data-lucide="eye" class="w-4 h-4"></i>
                            </button>
                            <button onclick="editBib(<?php echo htmlspecialchars(json_encode($bib)); ?>)" class="p-2 hover:bg-vercel-gray-100 dark:hover:bg-vercel-gray-800 rounded-md text-vercel-gray-400 hover:text-vercel-black dark:hover:text-vercel-white transition-colors">
                                <i data-lucide="edit-3" class="w-4 h-4"></i>
                            </button>
                            <button onclick="confirmDelete(<?php echo $bib['id']; ?>)" class="p-2 hover:bg-vercel-gray-100 dark:hover:bg-vercel-gray-800 rounded-md text-vercel-gray-400 hover:text-vercel-red transition-colors">
    ...
    <?php if ($totalPages > 1): ?>
        <div class="flex items-center justify-between border-t border-vercel-gray-200 dark:border-vercel-gray-800 pt-8 mt-4">
            <div class="text-xs text-vercel-gray-400 font-medium">
                Showing <span class="text-vercel-black dark:text-vercel-white font-bold"><?php echo $offset + 1; ?></span> to <span class="text-vercel-black dark:text-vercel-white font-bold"><?php echo min($total, $offset + $perPage); ?></span> of <span class="text-vercel-black dark:text-vercel-white font-bold"><?php echo $total; ?></span> bibliographies
            </div>
...
<script>
    ...
    const MODAL_CLASSES = {
        ...
        btnPrimary: 'px-6 py-2 bg-vercel-black dark:bg-vercel-white text-white dark:text-vercel-black rounded-md font-bold text-sm hover:bg-vercel-gray-800 dark:hover:bg-vercel-gray-100 transition-all shadow-sm shadow-black/10',
            ...
            content: `
                <div class="space-y-8 py-6">
                    <div class="p-8 border border-vercel-gray-200 dark:border-vercel-gray-800 rounded-lg">
                        <div class="flex items-center gap-4 mb-6 pb-6 border-b border-vercel-gray-100 dark:border-vercel-gray-800">
                            <div class="w-10 h-10 border border-vercel-gray-200 dark:border-vercel-gray-800 rounded flex items-center justify-center text-vercel-gray-400">
                                <i data-lucide="${getLucideIcon(bib.rt_icon)}" class="w-5 h-5"></i>
                            </div>
                            <div>
                                <h4 class="text-sm font-black text-vercel-black dark:text-vercel-white uppercase tracking-tight">${'<?php echo $currentLang; ?>' === 'th' ? bib.name_th : bib.name_en}</h4>
                                <p class="text-[11px] text-vercel-gray-400 font-bold uppercase tracking-widest mt-1">By @${bib.username}</p>
                            </div>
                        </div>
                        <div class="text-sm font-medium leading-relaxed text-vercel-black dark:text-vercel-white italic bg-vercel-gray-100/30 dark:bg-vercel-gray-800/30 p-4 rounded-md border border-vercel-gray-100 dark:border-vercel-gray-800">
                            ${bib.bibliography_text}
                        </div>
                    </div>
                    <div class="grid grid-cols-2 gap-px bg-vercel-gray-200 dark:bg-vercel-gray-800 rounded border border-vercel-gray-200 dark:border-vercel-gray-800 overflow-hidden">
                        <div class="bg-white dark:bg-vercel-gray-900 p-6">
                            <div class="${MODAL_CLASSES.label}">Created date</div>
                            <div class="text-sm font-bold text-vercel-black dark:text-vercel-white">${bib.created_at}</div>
                        </div>
                         <div class="bg-white dark:bg-vercel-gray-900 p-6">
                            <div class="${MODAL_CLASSES.label}">Language</div>
                            <div class="text-sm font-bold text-vercel-black dark:text-vercel-white uppercase tracking-widest">${bib.language}</div>
                        </div>
                    </div>
                </div>
            `,
        ...
        Modal.create({
            ...
            onOpen: (m) => {
                m.querySelector('#exec-del-bib').onclick = async () => {
                    ...
                    try {
                        const res = await API.delete('<?php echo SITE_URL; ?>/api/bibliography/delete.php', {
                            id: id
                        });
                        if (res.success) {
