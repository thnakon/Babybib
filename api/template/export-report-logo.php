 ...
 * ===========================================================
 * POST /api/template/export-report-logo.php
 * 
 * Uses PHPOffice/PHPWord TemplateProcessor to clone blocks 
 * and export an academic_general_logo report.
 */
...
$templateId = $payload['template'] ?? 'academic_general_logo';
$format = strtolower($payload['format'] ?? 'docx');
$coverData = $payload['coverData'] ?? [];
$projectId = intval($payload['projectId'] ?? 0);

// Dedicated endpoint for academic_general_logo only
if ($templateId !== 'academic_general_logo') {
    http_response_code(400);
    die('รองรับเฉพาะรายงานวิชาการทั่วไป พร้อม Logo ในตอนนี้');
}
...
$templatePath = __DIR__ . '/../../assets/templates/template_academic_general_logo.docx';
...
// 1.5 Resolve logo image from preview upload/default
$logoTempFile = null;
...
if ($logoMeta && !empty($logoMeta['bytes'])) {
    ...
    if ($logoTempFile !== false) {
        ...
        if (@file_put_contents($logoPathWithExt, $logoMeta['bytes']) !== false) {
            ...
            // Replace all ${cover_logo} placeholders on the cover pages.
            $templateProcessor->setImageValue('cover_logo', [
                'path' => $logoTempFile,
                // Standard cover logo size ~3.4 cm (about 130 px at 96dpi)
                'width' => 130,
                'height' => 130,
                'ratio' => true
            ]);
...
header('Content-Disposition: attachment; filename="report-academic_general_logo.docx"');
...
readfile($tempFile);
unlink($tempFile);
if ($logoTempFile && is_file($logoTempFile)) {
    @unlink($logoTempFile);
}
exit;
